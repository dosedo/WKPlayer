...
@implementation WKVideoFrame

+ (instancetype)frame
{
    static NSString *name = @"WKVideoFrame";
...
{
    WKVideoFrame *frame = [WKVideoFrame frame];
    uint8_t *data[WKFramePlaneCount] = {nil};
    int linesize[WKFramePlaneCount] = {0};
...
- (WKPLFImage *)image
{
    WKRational presentationSize = self->_descriptor.presentationSize;
    if (presentationSize.num == 0 ||
        presentationSize.den == 0) {
        return nil;
    }
    WKVideoDescriptor *descriptor = [[WKVideoDescriptor alloc] init];
    descriptor.format = AV_PIX_FMT_RGB24;
    descriptor.width = presentationSize.num;
    descriptor.height = presentationSize.den;
    const uint8_t *src_data[WKFramePlaneCount] = {nil};
    uint8_t *dst_data[WKFramePlaneCount] = {nil};
    int src_linesize[WKFramePlaneCount] = {0};
    int dst_linesize[WKFramePlaneCount] = {0};
    ...
    WKSWScale *context = [[WKSWScale alloc] init];
    ...
    WKPLFImage *image = WKPLFImageWithRGBData(dst_data[0],
                                              dst_linesize[0],
                                              descriptor.width,
                                              descriptor.height);
    av_freep(dst_data);
...
- (void)fillWithFrame:(WKFrame *)frame
{
    ...
    WKVideoFrame *videoFrame = (WKVideoFrame *)frame;
...
{
    AVFrame *frame = self.core;
    self->_descriptor = [[WKVideoDescriptor alloc] initWithFrame:frame];
