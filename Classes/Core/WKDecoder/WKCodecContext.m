...
@property (nonatomic, copy, readonly) __kindof WKFrame *(^frameGenerator)(void);

@end

@implementation WKCodecContext
                  ...
                  frameGenerator:(__kindof WKFrame *(^)(void))frameGenerator
{
    if (self = [super init]) {
        ...
        self->_options = [WKOptions sharedOptions].decoder.copy;
    }
    return self;
}
...
- (NSArray<__kindof WKFrame *> *)decode:(WKPacket *)packet
{
    ...
    while (result != AVERROR(EAGAIN)) {
        __kindof WKFrame *frame = self->_frameGenerator();
...
- (AVCodecContext *)createCcodecContext
{
    ...
    NSError *error = WKGetFFError(result, WKActionCodeCodecSetParametersToContext);
    if (error) {
    ...
    if ((self->_options.hardwareDecodeH264 && self->_codecpar->codec_id == AV_CODEC_ID_H264) ||
        ...
        codecContext->get_format = WKCodecContextGetFormat;
    }
    ...
    AVDictionary *opts = WKDictionaryNS2FF(self->_options.options);
    ...
    error = WKGetFFError(result, WKActionCodeCodecOpen2);
    if (error) {
        avcodec_free_context(&codecContext);
        return nil;
    }
...
static enum AVPixelFormat WKCodecContextGetFormat(struct AVCodecContext *s, const enum AVPixelFormat *fmt)
{
    WKCodecContext *self = (__bridge WKCodecContext *)s->opaque;
...
//            frames_ctx_data->sw_format = WKPixelFormatAV2FF(self->_options.preferredCVPixelFormat);
