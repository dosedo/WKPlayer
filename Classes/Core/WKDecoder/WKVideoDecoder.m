...
@property (nonatomic, strong, readonly) WKSWScale *scaler;
@property (nonatomic, strong, readonly) WKCodecContext *codecContext;
@property (nonatomic, strong, readonly) WKVideoFrame *lastDecodeFrame;
@property (nonatomic, strong, readonly) WKVideoFrame *lastOutputFrame;
@property (nonatomic, strong, readonly) WKCodecDescriptor *codecDescriptor;

@end

@implementation WKVideoDecoder
    ...
    if (frames.count == 0 &&
        ...
        if (CMTIME_IS_NUMERIC(timeRange.start) &&
            ...
            if (CMTimeCompare(duration, kCMTimeZero) > 0) {
                WKVideoFrame *obj = [WKVideoFrame frame];
...
- (NSArray<__kindof WKFrame *> *)processFrames:(NSArray<__kindof WKFrame *> *)frames done:(BOOL)done
{
    ...
    for (WKVideoFrame *obj in frames) {
...
- (NSArray<__kindof WKFrame *> *)clipKeyFrames:(NSArray<__kindof WKFrame *> *)frames
{
...
- (NSArray<__kindof WKFrame *> *)formatFrames:(NSArray<__kindof WKFrame *> *)frames
{
    ...
    for (WKVideoFrame *obj in frames) {
        ...
        if (![self->_scaler.inputDescriptor isEqualToDescriptor:obj.descriptor]) {
            WKSWScale *scaler = [[WKSWScale alloc] init];
        ...
        WKVideoFrame *newObj = [WKVideoFrame frameWithDescriptor:self->_scaler.outputDescriptor];
...
- (NSArray<__kindof WKFrame *> *)resampleFrames:(NSArray<__kindof WKFrame *> *)frames
{
    ...
    for (WKVideoFrame *obj in frames) {
        ...
        while (CMTimeCompare(frame.duration, frameRate) > 0) {
            ...
            WKVideoFrame *newFrame = [WKVideoFrame frame];
