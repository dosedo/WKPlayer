...
@implementation WKVideoDescriptor

- (id)copyWithZone:(NSZone *)zone
{
    WKVideoDescriptor *obj = [[WKVideoDescriptor alloc] init];
...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        self->_cv_format = WKPixelFormatFF2AV(self->_format);
        self->_width = 0;
        self->_height = 0;
        self->_sampleAspectRatio = (WKRational){0, 1};
    }
    return self;
}
...
{
    self->_format = format;
    self->_cv_format = WKPixelFormatFF2AV(format);
}

- (void)setCv_format:(OSType)cv_format
{
    self->_format = WKPixelFormatAV2FF(cv_format);
    self->_cv_format = cv_format;
}

- (WKRational)frameSize
{
    return (WKRational){self->_width, self->_height};
}

- (WKRational)presentationSize
{
    ...
    WKRational size1 = {width, av_rescale(width, aspectRatio.den, aspectRatio.num) & ~1};
    WKRational size2 = {av_rescale(height, aspectRatio.num, aspectRatio.den) & ~1, height};
